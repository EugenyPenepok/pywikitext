...
class Fragment:
    ...
    def genGrammar(self):
        ...
        # Класс для доступа к конфигурационному файлу типов СФЭ
...
# Класс для итерации по разделам статей, заданных в конфигурации
class AbstractFragmentIterator(metaclass=ABCMeta):
    ...
    def build(self):
        self.preProcess()
        for fType in self.fragmentTypesToHeaders.keys():
            print("Process " + fType)
            self.processFragmentStart(fType)
            docCount = 0
            for header in self.fragmentTypesToHeaders[fType]:
                headerId = int(header)
...
# Функция построения гистограммы по токенам
def calcHist(tokens):
...
# Функция построения гистограмм типов СФЭ
class POSListBuilder(AbstractFragmentIterator):
    def __init__(self, accessor, indexPrefix, configuration=None):
        super(POSListBuilder, self).__init__(accessor, indexPrefix, configuration)
...
# Функция доступа к индексу гистограмм частей речи типов СФЭ
...
# Функция получения множества статей для обработки
...
# Функция составления списка заголовков
...
# Функция построения индекса клише
...
# Функция построения статистики для визуализации
...
# Функция построения индекса гистограмм частей речи
